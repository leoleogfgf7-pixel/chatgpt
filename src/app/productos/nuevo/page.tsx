...
export default function NuevoProducto() {
  ...
  useEffect(() => { fetch("/api/setup").catch(() => {}).finally(() => { fetch("/api/categorias").then(r => r.ok ? r.json() : []).then(setCats).catch(() => {}); }); }, []);
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">Agrega un producto a tu inventario</p>
      </div>
      <div className="glass-strong rounded-2xl p-6 space-y-5">
        <div className="grid md:grid-cols-2 gap-4">
          <DarkField label="Nombre *" value={form.nombre} onChange={v => set("nombre", v)} />
          <DarkField label="SKU" value={form.sku} onChange={v => set("sku", v)} placeholder="Código único" />
        </div>
        ...
        <div className="border-t border-white/5 pt-5">
          ...
          <div className="flex gap-4 mt-3 text-sm">
            ...
            {form.precioEspecial && (
              <span className="text-gray-500">Margen especial: <span className="font-semibold text-amber-400">{formatSoles(Number(form.precioEspecial) - Number(form.precioCosto || 0))}</span></span>
            )}
          </div>
        </div>
        ...
        <div className="border-t border-white/5 pt-5">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <DarkField label="Stock Inicial" value={form.stock} onChange={v => set("stock", v)} type="number" />
            <DarkField label="Stock Mínimo (alerta)" value={form.stockMinimo} onChange={v => set("stockMinimo", v)} type="number" />
          </div>
        </div>
        ...
        <div className="flex gap-3 pt-3">
          ...
          <button onClick={() => router.push("/productos")} className="glass hover:bg-white/10 px-6 py-2.5 rounded-xl font-medium transition">Cancelar</button>
        </div>
      </div>
    </div>
  );
